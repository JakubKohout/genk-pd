...
3. **Geography** (`/geo`) — interactive map of Los Santos and Blaine County
   ...
   **87 POI dataset**
   (`pois.ts` + `streets.generated.ts`): 54 city + 11 state + 13 street +
  ...
  modules/
    ...
    geo/                            # Geography module (interactive map + 2 sub-modes + position editor)
      data/
        ...
        pois.ts                     # POIS — 87 POI: 54 city + 11 state (point) + 22 street/highway
        ...
        streets.generated.ts        # HAND-TRACED from the art (not generated despite the name) —
                                    # 22 street polyline centerlines per the labels in the art
                                    # (Gotcha 41)
...
34. **Geo POI IDs in the E2E seed must be in sync with the data** — `e2e/fixtures/seed.ts`
    has a hardcoded `GEO_POI_IDS` (87 IDs, in `e2e/fixtures/geo-poi-ids.ts`). When adding a new POI to `pois.ts`
